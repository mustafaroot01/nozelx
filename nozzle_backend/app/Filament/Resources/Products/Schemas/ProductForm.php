                        ...
                        \Filament\Forms\Components\Tabs\Tab::make('المعلومات الأساسية')
                            ...
                            ->schema([
                                Select::make('category_id')
                                    ->label('القسم')
                                    ->options(function () {
                                        return \App\Models\Category::all()->mapWithKeys(function ($category) {
                                            $name = $category->name_ar ?? $category->name;
                                            if ($category->parent) {
                                                $parentName = $category->parent->name_ar ?? $category->parent->name;
                                    ...
                                    ->preload(),
                                Select::make('tags')
                                    ->label('التصنيفات المساعدة / العلامات واللزوجة')
                                    ->relationship('tags', 'name')
                                    ->multiple()
                                    ->options(function () {
                                        return \App\Models\ProductTag::all()->mapWithKeys(function ($tag) {
                                            $name = $tag->name;
                                            if ($tag->parent) {
